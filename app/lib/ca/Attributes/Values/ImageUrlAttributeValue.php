<?php
/** ---------------------------------------------------------------------
 * app/lib/ca/Attributes/Values/ImageUrlAttributeValue.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
 ...
 */
  ...
  *
  */
    define("__CA_ATTRIBUTE_VALUE_IMAGEURL__", 32);
 	...
 	$_ca_attribute_settings['ImageUrlAttributeValue'] = array(		// global
		'minChars' => array(
	...
	class ImageUrlAttributeValue extends AttributeValue implements IAttributeValue {
 		# ------------------------------------------------------------------
		...
		public function getDisplayValue($pa_options=null) {
 		...
 		public function htmlFormElement($pa_element_info, $pa_options=null) {
            ...
            AssetLoadManager::register('image');
			$vs_element = 	'<div id="imageContainer_'.$pa_element_info['element_id'].'_{n}" class="imageContainer">';
/*
			$vs_element .= 		'<div class="imageSearchBox" style="border-bottom: 1px solid #E6E6E6; padding-bottom:0.5em;  margin-bottom:0.5em;">';
			$vs_element .=			'<input type="text" class="imageSearchField" name="imageSearchField_{n}" size="60" value="'._t('PID van het originele item').'" autocomplete="off"/ style="margin: 0 10px 0 0;">';
			$vs_element .=			'<a href="#" class="button" id="searchImageButton">'._t('Search Image').' &rsaquo;</a>';
			$vs_element .= 		'</div>';

			$vs_element .= 		'<div id="image_search_thumbnails">';
			$vs_element .= 		'</div>';
*
*/

			$vs_element .=		'Image PID: <input type="text" class="imagePID" name="{fieldNamePrefix}'.$pa_element_info['element_id'].'_{n}" size="60" style="margin: 0 10px 0 0;">';
			$vs_element .= 		'<div id="image_thumbnail" style="padding:0.5em;">';
			$vs_element .= 		'</div>';
			...
			$vs_element .= 		"jQuery(document).ready(function() {
									...
									imagedata = initImageApp(imagedata);
																		
									if ('{n}'.substring(0,3) == 'new') {
										initNewImage(imagedata);
									} else {
 		...
 		public function getAvailableSettings($pa_element_info=null) {
 			global $_ca_attribute_settings;
 			return $_ca_attribute_settings['ImageUrlAttributeValue'];
 		}
 		# ------------------------------------------------------------------
		/**
        ...
        public function getType() {
            return __CA_ATTRIBUTE_VALUE_IMAGEURL__;
        }
        # ------------------------------------------------------------------
    }
 ?>
